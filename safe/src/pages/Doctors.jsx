...
const Doctors = () => {
    ...
    async function fetchDoctorsData() {
      ...
      try {
        ...
        for (let address of doctorAddresses) {
          try {
            const doctorHash = await mycontract.methods.getDoctor(address).call();
            ...
            console.error(`Error fetching doctor data for address ${address}:`, addressError);
          }
        }
  ...
  const handleAddDoctor = async (doctor) => {
    try {
        ...
        await mycontract.methods.addPatient(newHash).send({ from: walletAddress });
                  ...
                  {filteredDoctors.map((doctor) => (
                    ...
                    >
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <Chip 
                          label={selectedDoctors.has(doctor.address) ? "Added" : "Available"} 
                          color={getStatusChipColor(selectedDoctors.has(doctor.address))}
                          size="small"
                        />
                      </div>
          ...
          {selectedDoctor && (
            <>
              ...
              <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
                ...
                <Typography variant="body1">{selectedDoctor.speciality}</Typography>
              </Paper>
              
              <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
                <Typography variant="body2" color="textSecondary">Status</Typography>
                <Chip 
                  label={selectedDoctors.has(selectedDoctor.address) ? "Added" : "Available"} 
                  color={getStatusChipColor(selectedDoctors.has(selectedDoctor.address))}
                  size="small"
                />
              </Paper>
